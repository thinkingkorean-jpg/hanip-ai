...
{{
  ...
  "recommended_tools": [
    ...
    // 업무나 일상에 도움되는 AI 도구 2개를 소개하세요.
  ],
  "hannip_comment": "한입이의 재치있는 마무리 한마디 (2~3문장, 이모지 듬뿍 포함, 오늘의 메인 테마를 요약하며 인사)"
}}
```

중요: 반드시 유효한 JSON으로 작성하세요. 큰따옴표(")가 문자열 내부에 들어갈 경우 반드시 백슬래시(\")로 이스케이프(escape) 처리해야 합니다! 각 `deep_dives` 본문은 반드시 500자 이상의 장문이어야 합니다!
"""
...
def init_gemini():
    ...
    return genai.GenerativeModel(
        ...
        tools=[{"function_declarations":[]}] # Optional strict JSON parsing
    )
